...
기본 endpoint는 `http://localhost:8001/mcp`입니다. 별도 터미널에서 Inspector를
실행한 뒤 전송 방식으로 Streamable HTTP를 선택하고 이 URL에 연결합니다.
...
E2E 테스트는 백엔드와 MCP 서버를 `NEIS_FIXTURE_MODE=true`로 실행해 실제
NEIS API 키 없이도 결정적으로 동작합니다. 아래처럼 애플리케이션을 실행한
상태에서 `E2E_BASE_URL`(기본값 `http://localhost:5173`)을 대상으로 테스트를
수행합니다.
